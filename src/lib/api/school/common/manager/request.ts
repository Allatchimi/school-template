...
export interface ManagerRequest extends BaseRequest {
...
export interface ManagerListRequest extends FilterRequest, PaginationRequest {
  schoolID?: IDType | null;
}

// Format request
export function formatManagerFormToRequest(item?: ManagerRequest) {
  if (!item) {
    return;
  }
...
export function compareManagerRequestToResponse(
  a?: ManagerRequest,
  b?: ManagerResponse
) {
  if (!a || !b) {
  ...
  const tempReq = formatManagerFormToRequest(a);

  return (
    tempReq?.schoolID === b.school?.id &&
    tempReq?.roleID === b.user?.role?.id &&
    tempReq?.uid === b.uid &&

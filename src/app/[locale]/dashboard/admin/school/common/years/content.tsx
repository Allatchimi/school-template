"use client";

import { DescriptionSchool } from "@/components/description/school/common/description-school";
import { DescriptionYear } from "@/components/description/school/common/description-year";
import {
  FilterSchoolType,
  filterSchoolTypeTemplate,
} from "@/components/filter/default-filters";
import FormItemSelectSchool from "@/components/form-item/select/school/common/select-school";
import FormAddUpdateYear from "@/components/form/school/common/form-add-update-year";
import { TableColumsYear } from "@/components/table/columns/school/common/columns-year";
import ContentTableTemplate from "@/components/template/content-table/content-table-template";
import { SchoolConfig } from "@/config/school";
import { formatSortStrToSortOrder } from "@/helpers/cast/table";
import {
  YearRequest,
  YearListRequest,
  compareYearRequestToResponse,
} from "@/lib/api/school/common/year/request";
import {
  YearResponse,
  YearListResponse,
} from "@/lib/api/school/common/year/response";
import {
  getYearList,
  postYear,
  updateYear,
  deleteYear,
  deleteMultipleYear,
} from "@/lib/api/school/common/year/routes";
import { useTranslations } from "next-intl";
import { useSearchParams } from "next/navigation";

export default function PageContent() {
  // Next hooks
  const tWords = useTranslations("Words");

  // React hooks
  const searchParams = useSearchParams();
  const paramOrderBy = searchParams.get("orderBy");
  const paramSort = searchParams.get("sort");

  return (
    <>
      <ContentTableTemplate<
        YearRequest,
        YearResponse,
        YearListRequest,
        YearListResponse,
        FilterSchoolType
      >
        itemLabel={tWords("year")}
        downloadUploadTableName="years"
        queryKeyData="admin-years-data"
        canAdd={true}
        canUpdate={true}
        canDeleteOne={true}
        canDeleteMultiple={true}
        canSelectMultiple={true}
        canUpload={false}
        canDownload={false}
        addDefaultActions={true}
        dialogFormAddUpdateWidth={600}
        dialogDescriptionWidth={800}
        searchParams={searchParams}
        filterTemplate={filterSchoolTypeTemplate}
        returnExtraFilterNode={(values, loading) => {
          if ((SchoolConfig.schoolID() ?? 0) > 0) {
            return undefined;
          }
          return (
            <div className="w-auto flex flex-wrap items-center gap-4">
              <FormItemSelectSchool
                disabled={loading}
                defaultValue={values?.schoolID?.toString()}
                allowEmptySelection={true}
                allowEmptySelectionLabel="*"
                noMargin={true}
              />
            </div>
          );
        }}
        columns={TableColumsYear({
          schoolType: SchoolConfig.schoolType(),
          yearID: SchoolConfig.yearID(),
          orderBy: paramOrderBy ?? undefined,
          sort: formatSortStrToSortOrder(paramSort ?? undefined),
        })}
        areEqual={compareYearRequestToResponse}
        returnFormAddUpdateNode={FormAddUpdateYear}
        returnDescriptions={(item) => {
          return [
            { description: DescriptionYear(item) },
            {
              title: tWords("school"),
              description: DescriptionSchool(item?.school ?? undefined),
            },
          ];
        }}
        getItemList={getYearList}
        postItem={postYear}
        updateItem={updateYear}
        deleteItem={deleteYear}
        deleteMultipleItems={deleteMultipleYear}
        uploadData={undefined}
        downloadData={undefined}
      />
    </>
  );
}

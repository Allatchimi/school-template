...
import { StudentRequest } from "@/lib/api/school/common/student/request";
...
export default function FormAddUpdateStudent(
  props: CustomFormProps<StudentRequest, StudentResponse>
) {
  // React hooks
  const [form] = useForm<StudentRequest>();
  const initialRequest: StudentRequest = {
    schoolID: SchoolConfig.schoolID(),
    autoGenerateEmail: true,
  };
  const [request, setRequest] = useState<StudentRequest | undefined>({
  ...
  const handleValuesChange = (values: StudentRequest) => {
  ...
  const handleFinish = (values: StudentRequest) => {
    if (props.onSubmit) {
      const newValues = values;
      if (initialRequest?.schoolID) {
        newValues.schoolID = initialRequest.schoolID;

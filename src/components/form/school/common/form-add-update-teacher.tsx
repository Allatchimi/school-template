...
import { TeacherRequest } from "@/lib/api/school/common/teacher/request";
...
export default function FormAddUpdateTeacher(
  props: CustomFormProps<TeacherRequest, TeacherResponse>
) {
  // React hooks
  const [form] = useForm<TeacherRequest>();
  const initialRequest: TeacherRequest = {
    schoolID: SchoolConfig.schoolID(),
    autoGenerateEmail: true,
  };
  const [request, setRequest] = useState<TeacherRequest | undefined>({
  ...
  const handleValuesChange = (values: TeacherRequest) => {
  ...
  const handleFinish = (values: TeacherRequest) => {
    if (props.onSubmit) {
      const newValues = values;
      if (initialRequest?.schoolID) {
        newValues.schoolID = initialRequest.schoolID;
  ...
  return (
    <CustomForm
      {...props}
      form={form}
      name={props.formName ?? "form-add-update-teacher"}
      layout={"vertical"}
      className="w-full"

...
import { SemesterRequest } from "@/lib/api/school/university/semester/request";
...
export default function FormAddUpdateSemester(
  props: CustomFormProps<SemesterRequest, SemesterResponse>
) {
  // React hooks
  const [form] = useForm<SemesterRequest>();
  const initialRequest: SemesterRequest = {
    schoolID: SchoolConfig.schoolID(),
  };
  const [, setRequest] = useState<SemesterRequest | undefined>({
  ...
  const handleValuesChange = (values: SemesterRequest) => {
  ...
  const handleFinish = (values: SemesterRequest) => {
    if (props.onSubmit) {
      const newValues = values;
      if (initialRequest?.schoolID) {
        newValues.schoolID = initialRequest.schoolID;
  ...
  return (
    <CustomForm
      {...props}
      form={form}
      name={props.formName ?? "form-add-update-semester"}
      layout={"vertical"}
      className="w-full"

...
export default function FormAddUpdateFaculty(
  props: CustomFormProps<FacultyRequest, FacultyResponse>
) {
  // React hooks
  const [form] = useForm<FacultyRequest>();
  const initialRequest: FacultyRequest = {
    schoolID: SchoolConfig.schoolID(),
  };
  const [, setRequest] = useState<FacultyRequest | undefined>({
  ...
  const handleValuesChange = (values: FacultyRequest) => {
  ...
  const handleFinish = (values: FacultyRequest) => {
    if (props.onSubmit) {
      const newValues = values;
      if (initialRequest?.schoolID) {
        newValues.schoolID = initialRequest.schoolID;
  ...
  return (
    <CustomForm
      {...props}
      form={form}
      name={props.formName ?? "form-add-update-faculty"}
      layout={"vertical"}
      className="w-full"
